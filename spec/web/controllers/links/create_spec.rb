...
RSpec.describe Web::Controllers::Links::Create do
  ...
  context 'when url is valid' do
    ...
    it 'creates new link object' do
      expect{ action.call(params) }.to change { repo.count }.by(1)
    ...
    it 'displays link in flash' do
      ...
      it(flash[:info]).to match repo.last.key
    end
  end
  ...
  context 'when url is invalid' do
    ...
    it 'does not create new link object' do
      expect{ action.call(params) }.to change { repo.count }.by(0)
    end
    ...
    it 'displays error in flash' do
      action.call(params)
      flash = action.exposures[:flash]
      it(flash[:error]).to eq 'invalid link'
    end
  end
end
